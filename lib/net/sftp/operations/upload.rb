  ...
  class Upload
      ...
      if recursive?
        ...
        if @options[:create_dir]
          sftp.mkdir(remote) do |response|
            @active -= 1
            raise StatusException.new(response, "mkdir `#{remote}'") unless response.ok || response.code == 4
    ...
    private
      ...
      def on_mkdir(response)
        ...
        raise StatusException.new(response, "mkdir #{dir}") unless response.ok? || response.code == 4
